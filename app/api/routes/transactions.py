...
from app.exceptions.exceptions import EntityAlreadyExistsError, ServiceError, EntityDoesNotExistError
from app.models.transaction import (TransactionModel, UpdateTransactionModel, TrasactionCollection, ResponseModel)
...
async def add_transaction_record(transaction: TransactionModel = Body(...)):
    ...
    try:
        ...
        return ResponseModel(new_transaction, "Transaction added successfully.", status.HTTP_201_CREATED)
    except Exception as e:
        logger.error("An error occurred while adding a transaction record", e)
        raise ServiceError("An error occurred while adding a transaction record")
...
async def get_transaction_data(id: str):
    ...
    try:
        ...
        return ResponseModel(transaction, "Transaction data retrieved successfully", status.HTTP_200_OK)
    except EntityDoesNotExistError as e:
        logger.error(f"Transaction not found for ID: {id}")
        raise EntityDoesNotExistError(f"Transaction with id: {id} not found.")
    except Exception as e:
        logger.error(f"An error occurred while retrieving transaction data for ID: {id}", e)
...
async def get_transaction_history(user_id: str):
    ...
    try:
        ...
        return ResponseModel(transaction_history, "Transaction history retrieved successfully", status.HTTP_200_OK)
    except EntityDoesNotExistError as e:
        logger.error(f"Transaction history not found for user ID: {user_id}")
        raise EntityDoesNotExistError(f"Transaction history not found for user ID {user_id}")
    except Exception as e:
        logger.error(f"An error occurred while retrieving transaction history for user ID: {user_id}", e)
        raise ServiceError()

@router.put("/{id}", response_description="Transaction data updated", response_model=ResponseModel)
async def update_transaction_data(id: str, req: UpdateTransactionModel = Body(...)):
    ...
    try:
        ...
        if updated_transaction:
            ...
            return ResponseModel(f"Transaction with ID: {id} update is successful", "Transaction updated successfully", status.HTTP_200_OK)
        else:
...
@router.delete("/{id}", response_description="Transaction data deleted from the database", response_model=ResponseModel)
    ...
    try:
        ...
        if deleted_transaction:
            ...
            return ResponseModel(
                f"Transaction with ID: {id} removed", "Transaction deleted successfully", status.HTTP_200_OK
            )
        else:
            logger.warning(f"Transaction with ID: {id} not found")
            raise EntityDoesNotExistError(f"Transaction with id: {id} not found.")
        ...
        logger.error(f"An error occurred while deleting transaction data for ID: {id}", exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="An error occurred while deleting transaction data")