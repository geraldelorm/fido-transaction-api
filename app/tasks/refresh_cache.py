...
from app.crud.transactions_service import fetch_transaction_history_from_db, fetch_transaction_analytics_from_db
...
async def refresh_cache(user_id: str):
    try:
        ...
        redis_client.setex(f"transaction_history:{user_id}", CACHE_EXPIRATION, json.dumps(transaction_history))
        
        # Refresh transaction analytics cache
        transaction_analytics = await fetch_transaction_analytics_from_db(user_id)
        redis_client.setex(f"transaction_analytics:{user_id}", CACHE_EXPIRATION, json.dumps(transaction_analytics))
        
        logger.info(f"Cache refreshed for user ID: {user_id}")
    except Exception as e:
        logger.error(f"An error occurred while refreshing cache for user ID: {user_id}: {e}")